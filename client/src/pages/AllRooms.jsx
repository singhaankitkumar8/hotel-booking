...
const AllRooms = () => {
    ...
    const [searchParams , setSearchParams] = useSearchParams();    //from the url we will get search parameters
    ...
    //Function to check if sort rooms based on selected sort option
    const sortRooms = (a,b) => {
  ...
  return (
    <div className='flex flex-col reverse lg:flex-row items-start justify-between pt-28 md:pt-35 px-4 md:px-16 lg:px-24'>
              ...
              {filteredRooms.map((room) => (
                  <div key={room._id} className='flex flex-col md:flex-row items-start py-10 gap-6 border-b border-gray-300 last:pb-30 last:border-0'>
                      ...
                      <div className='md:w-1/2 flex flex-col gap-2'>
                          ...
                          {/* Room Amenities */}
                          <div className='flex flex-wrap items_center mt-3  mb-6 gap-4'>
                              {room.amenities.map((item, index) => (
                                  <div key={index} className='flex items-center gap-2 px-3 py-2 rouded-lg bg-[#F5F5FF]/70'>
          ...
          <div className='bg-white w-80 border border-gray-300 text-gray-600 max-lg:mb-8 lg:mt-16'>
              ...
              <div className={`${openFilters ? 'h-auto' : 'h-0 lg:h-auto'} overflow-auto transition-all duration-700`}>
                  ...
                  <div className='px-5 pt-5'>
                      ...
                      {priceRanges.map((range , index) => (
                          <CheckBox key={index} label={`${currency} ${range}`} selected={selectedFilters.priceRange.includes(range)} onChange={(checked) => handleFilterChange(checked,room,'roomType')}/>
                      ))}
                  </div>
